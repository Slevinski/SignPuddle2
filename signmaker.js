/*
//@+leo-ver=4-thin
//@+node:slevin.20070119105433.2:@thin W:/www/signmaker.js
//@@first
//@delims /* */ 
/*@@language java*/
//common variables
...
function buildControl() {
  ...
  for (var i=0; i<signBox.pic.children.length;i++){
    build=build + signBox.pic.children[i].sss + "," + signBox.pic.children[i].getX() + "," + signBox.pic.children[i].getY() + ","
  }
  iGL = "<a href=\"#\" onclick=\"goTop();return false;\" >" + iGLi + "</a>";
  iPR = "<a href=\"#\" onclick=\"goUp();return false;\">" + iPRi + "</a>";

  if (sid){
    iAD = "<a href=\"canvas.php?ui=" + uiVal + "&sgn=" + sgnVal + "&build=" + build;
    iAD += "&sid=" + sid + "&action=rewrite";
    iAD +=  "\">" + iADi + "</a>";
//    aOut=aOut+"<form name='options' action=\"canvas.php\" method=\"POST\"><input type=\"hidden\"  name=\"action\" value=\"Save\"><button type=submit>" + iSAVEi + "</button><input type=hidden name=list><input type=hidden name=ui value=\"" + uiVal + "\"><input type=hidden name=sgn value=\"" + sgnVal + "\"><input type=hidden name=sid value=\"" + sid + "\"></form>"
    ...
    iAD = "<a href=\"signsave.php?ui=" + uiVal + "&sgn=" + sgnVal + "&build=" + build;
    iAD +=  "\">" + iADi + "</a>";
  }
  ...
  iSC = "<a href=\"#\" onclick=\"standardColors();return false;\" >" + iSCi + "</a>";
  iPC = "<a href=\"#\" onclick=\"cp.show('pick');return false;\" name=\"pick\" id=\"pick\">" + iPCi + "</a>";
  ...
  atab=atab + "<tr><td>" +iSN+ "</td><td>" +iSC+ "</td><td>" +iPC+ "</td></tr>";
  atab=atab + "</table>";
...
function symbolNext() {
  ...
  if(sbSymbol>0) {signBox.pic.children[sbSymbol-1].select()} 
}
...
function idSymbol(){
  for (var i=0; i<signBox.pic.children.length;i++){
    if (this==signBox.pic.children[i]) {
      id=i+1
    }
  }
  return id
...
function selectSymbol(){
  if (this.selected==1) {
    ...
    if (sbLast>0 && sbLast<=signBox.pic.children.length) {
      signBox.pic.children[sbLast-1].off() 
    }
    this.on()
    sbSymbol=this.sid()
    sbLast=this.sid()
...
function onSymbol(){
  this.selected=1
  this.setHTML("<img alt='" + this.sss + "' src='" + swis_glyph + "?sss=" + this.sss + "&color=6666ff"+ "' onload='resizeSymbolX(" + this.cID + ");'>")
} 

...
echo 'Found ' . count($words) . ' sign(s)';
//with ' . $output[2] . ' occurance(s)';
echo ', <a href="searchquery.php?ui=' . $ui . '&sgn=' . $sgn . '&qsearch=' . $tquery . '">';
echo 'see the signs</a> or refine search criteria below'; 
if ($trange){
  $input = implode(' ',$words);
  ...
  echo "<h2>Sign counts for BaseSymbols</h2>";
  ...
  for ($row=0;$row<10;$row++){
    ...
    for ($col=0;$col<$cols;$col++){
      ...
      $i = $row + $col*10;
      if ($i<$count) {
        echo '<a href="drilldown.php?qsearch=' . $bsinfo[$i][2] . '">';
        if ($bsinfo[$i][1]>0) {
...
if (!$trange) {
  ...
  //need a grid, top level...
  if (!$qsearch) $qsearch="Q";
  echo "<h2>Sign counts for SymbolGroups</h2>";
  echo "<p><table cellpadding=4 border=1>";
  for ($row=0;$row<10;$row++){
    echo "<tr>";
    for ($col=0;$col<3;$col++){
      echo "<td align=middle width=70>";
      $i = $row + $col*10;
      echo '<a href="drilldown.php?qsearch=' . $qsearch . '&trange=' . $sginfo[$i][2] . '">';
      if ($sginfo[$i][1]>0) {

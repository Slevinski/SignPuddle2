...
for ($i=1;$i<6;$i++){
  ...
  echo '<img border=0 src="' . $swis_glyph . '?key=' . $tCat . $glyph_line . '">';
  echo '<br>';
  ...
  if ($cat!=3){
    ...
    foreach ($sg_list as $symgrp){
      if (hexdec($symgrp)>= hexdec($sBase) && hexdec($symgrp)<= hexdec($eBase)){
        ...
        echo '<img border=0 src="' . $swis_glyph . '?key=' . base2view($symgrp) . '">';
        echo '</a></td><td>&nbsp;&nbsp;</td>';
      }
    }
    echo "</tr><tr>";
      ...
      for ($c=0;$c<10;$c++){
        ...
        if ($tBase <= hexdec($eBase)){
          ...
          if ($count){
            ...
            echo '<img border=0 src="' . $swis_glyph . '?key=' . base2view(dechex($tBase)) . '">';
            echo "</a>";
          } else {
            echo '<img border=0 src="' . $swis_glyph . '?key=' . base2view(dechex($tBase)) . '&line=aaaaaa">';
          }
        }
        echo "</td><td>&nbsp;&nbsp;</td>";
      }
        ...
        for ($c=0;$c<6;$c++){
          ...
          if (validKey($tBase)){
            ...
            if ($count){
              ...
              echo '<img border=0 src="' . $swis_glyph . '?key=' . $tBase . '">';
              echo "<br>(" . $count . ")";
              echo "</a>";
            } else {
              echo '<img border=0 src="' . $swis_glyph . '?key=' . $tBase . '&line=aaaaaa"><br><br>';
            }
          }
          echo "</td>";
        }
